...
sudo ip netns exec $1 ip addr add 192.168.$2.1/24 dev $1-client
sudo ip netns exec $1 ip link set $1-client up
sudo ip netns exec $1 ip link set lo up
sudo ip link set $1-host up
sudo ip route add 192.168.$2.1/32 dev $1-host
sudo ip netns exec $1 ip route add default via 192.168.$2.1 dev $1-client
