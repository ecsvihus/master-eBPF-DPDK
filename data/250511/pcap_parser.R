...
for (file in files) {
  ...
  #filename <- "xdp-9000-6.data"
  traffic_bandwidth <- as.numeric(strsplit(filename, "-")[[1]][2])
  type <- strsplit(filename, "-")[[1]][1]
  round <- strsplit(filename, "-")[[1]][3]
  
  tmp <- c(fread(file, col.names = "latency") %>% mutate(file = filename, traffic = sprintf("%05d",traffic_bandwidth), fwd_type = type, round = round))
  #tmp <- c(fread(file, col.names = "latency") %>% mutate(file = filename, traffic = traffic_bandwidth, fwd_type = type, round = round))
...
#NAPI only
# %>% filter(!grepl('xdp-9000-6.data',file))
ggplot(combined.data %>% filter(grepl('dpdk',file)) %>% filter(grepl('00000|3|6|9',traffic)), aes(x = latency, color = factor(traffic))) +
  #scale_color_manual(values=c("#0000b3","#0000ff","#6666ff")) +
  #scale_color_discrete()
  #scale_color_gradient()
  scale_color_viridis_d(name="Traffic (mbps)") +
  theme_bw(base_size = 20) +
  stat_ecdf() +
  xlab("Latency (μs)") + 
  #stat_density() +
...
ggplot(combined.data %>% filter(grepl('3000',file)) , aes(x = latency, color = fwd_type)) +
  #scale_color_manual(values=c("#0000b3","#0000ff","#6666ff")) +
  #scale_color_discrete()
  #scale_color_gradient()
#  scale_color_viridis_d(name="Traffic (mbps)") +
