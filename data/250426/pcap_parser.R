...
#combined.data <- bind_rows(tmp2)

combined.data <- data.frame()
for (file in files) {
  datapath_type <- strsplit(file, "/")[[1]][3]
  traffic_bandwidth <- strsplit(datapath_type, "-")[[1]][2]
  
  tmp <- c(fread(file, col.names = "latency") %>% mutate(type = datapath_type, traffic = traffic_bandwidth))
...
#NAPI only
ggplot(combined.data %>% filter(grepl('dpdk',type)), aes(x = latency, color = traffic)) +
...
#XDP only
ggplot(all.data %>% filter(grepl('XDP',type)), aes(x = latency, color = traffic)) +
